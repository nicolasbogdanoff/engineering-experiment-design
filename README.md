...
For model-building experiments, `add_two_factor_interactions` appends the pairwise coded interaction columns, while `decode_coded_design` maps the coded matrix back to physical low/high levels. These are explicit design-matrix operations; they do not automatically claim that an interaction is statistically significant.
